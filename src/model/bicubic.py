...
class Bicu(torch.nn.Module):
    def __init__(self, args):
        super(Bicu, self).__init__()
        self.scale = args.scale[0]
        self.conv_input = torch.nn.Conv2d(in_channels=3, out_channels=64, kernel_size=9, stride=1, padding=4, bias=False)

    def forward(self, x):
        y = imresize(x, scale=self.scale)
        return y
