...
from utility import draw_quant_hist
...
class quantization(nn.Module):
    def __init__(self, args=None, tag='fm', shape=[], feature_stride=None, logger=None):
        ...
        self.clip_moving_ave = AverageMeter()
    ...
    def update_quantization(self, **parameters):
        ...
        if not self.enable:
            ...
            # if self.index == 36 or self.index == 37:
        ...
        with torch.no_grad():
            if self.method == 'dorefa' and data is not None:
                ...
                if hasattr(self, 'clip_val') and isinstance(self.clip_val, torch.Tensor):
                    ...
                    # last_max_value = self.clip_val.item()  # simple moving ave
                    # max_value = (last_max_value + max_value) / 2
                    self.clip_moving_ave.update(max_value)
                    max_value = self.clip_moving_ave.avg
                    self.clip_val.fill_(max_value)
                    if self.iteration.data == self.args.stable:
                    # if self.iteration.data % self.args.stable == 999:
    ...
    def quantization_value(self, x, y):
        if self.iteration.data <= self.args.stable:
        # if self.iteration.data % self.args.stable == 999:
        #     print('quant_activation.iteration:' + str(self.iteration.item()))
            ...
            if self.quant_loss_enable and isinstance(self.quant_loss_function, nn.Module):
                if 'quant_loss' in self.args.global_buffer:
                    ...
                    # self.args.global_buffer['quant_loss'] += self.quant_loss_function(x.max(), self.clip_val).detach() * self.quant_loss_alpha
                else:
                    self.args.global_buffer['quant_loss'] = self.quant_loss_function(x, y) * self.quant_loss_alpha
                    # self.args.global_buffer['quant_loss'] = self.quant_loss_function(x.max(), self.clip_val).detach() * self.quant_loss_alpha
            return y
...
class custom_conv(nn.Conv2d):
    ...
    def forward(self, inputs):
        if not self.force_fp:
            draw_quant_hist(self.weight, self.quant_weight, 'wt_before_quant')
            weight = self.quant_weight(self.weight)
            draw_quant_hist(weight, self.quant_weight, 'wt_after_quant')
            ...
            if self.padding_after_quant:
                ...
                # draw_quant_hist(inputs[0], self.quant_activation, 'fm_before_pad')
                inputs = F.pad(inputs, _quadruple(self.pads), 'constant', 0)
                draw_quant_hist(inputs[0], self.quant_activation, 'fm_before_quant')
                inputs = self.quant_activation(inputs)
                draw_quant_hist(inputs[0], self.quant_activation, 'fm_after_quant')
        else:
            weight = self.weight
        ...
        if not self.force_fp:
            draw_quant_hist(output[0], self.quant_output, 'ot_before_quant')
            output = self.quant_output(output)
            draw_quant_hist(output[0], self.quant_output, 'ot_after_quant')
